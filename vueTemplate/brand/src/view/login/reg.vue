  ...
  <div class="container">
    <div>
      ...
      <div style="border-bottom: 2px solid #ccc;padding:1.5rem">
        <span class="l-title">欢迎注册浪博数据</span>
        <span>还没账号？</span>
          <span @click="goLogin" style="color: red;">马上登陆</span>
      </div>
      <div class="l-tab" style="text-align: center">
        <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
          <el-tab-pane label="手机号注册" name="first">
            <div style="padding: 1.5rem">
              <el-input v-model="dataFrom.mobile" placeholder="手机号/邮箱账号"></el-input>
              <el-input style="margin-top: 1rem" v-model="dataFrom.uname" placeholder="用户名"></el-input>
              <el-input style="margin-top: 1rem" v-model="dataFrom.upawd" type="password" placeholder="登陆密码"></el-input>
            </div>
            <div class="elbtn">
              <el-button @click="reg" >立即注册</el-button>
            </div>
          ...
          <el-tab-pane label="邮箱账号注册" name="second">
            123
          </el-tab-pane>
        </el-tabs>
      </div>
...
<script>
import { uRegister } from '@/api/login/login'
export default {
  data () {
    return {
      activeName: 'first',
  ...
  methods: {
    ...
    reg () {
      let data = {
        username: this.dataFrom.uname,
        password: this.dataFrom.upawd,
        mobile: this.dataFrom.mobile
      }
      uRegister(data).then(res => {
        if (res.data.errno === 0) {
          this.$message({
            message: '注册成功',
            center: true,
