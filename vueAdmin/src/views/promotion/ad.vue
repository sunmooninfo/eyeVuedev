<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="手机端图片" prop="appUrl">
        <template slot-scope="scope">
          <img v-if="scope.row.appUrl" :src="scope.row.appUrl" width="80">
        </template>
      </el-table-column>

      <el-table-column align="center" label="电脑端图片" prop="pcUrl">
        <template slot-scope="scope">
          <img v-if="scope.row.pcUrl" :src="scope.row.pcUrl" width="80">
        </template>
      </el-table-column>
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="电脑端图片" prop="pcUrl">
          <el-upload
            :headers="headers"
            :action="uploadPath"
            :show-file-list="false"
            :on-success="uploadUrlPC"
            :before-upload="checkFileSizePC"
            class="avatar-uploader"
            accept=".jpg,.jpeg,.png,.gif"
          >
            <img v-if="dataForm.pcUrl" :src="dataForm.pcUrl" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon" />
            <div slot="tip" class="el-upload__tip">建议像素比1200*550</div>
          </el-upload>
        </el-form-item>
        <el-form-item label="手机端图片" prop="appUrl">
          <el-upload
            :headers="headers"
            :action="uploadPath"
            :show-file-list="false"
            ...
            accept=".jpg,.jpeg,.png,.gif"
          >
            <img v-if="dataForm.appUrl" :src="dataForm.appUrl" class="avatar">
            <i v-else class="el-icon-plus avatar-uploader-icon" />
            <div slot="tip" class="el-upload__tip">建议像素比375*210</div>
          </el-upload>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    uploadUrlPC: function(response) {
      this.dataForm.pcUrl = response.data.url
    },
    uploadUrl: function(response) {
      this.dataForm.appUrl = response.data.url
    },
    checkFileSizePC: function(file) {
      if (file.size > 5242880) {
        this.$message.error(`${file.name}文件大于5M，请选择小于5M大小的图片`)
        return false
      }
      return true
    },
