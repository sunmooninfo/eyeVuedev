<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row row-key="id">
      ...
      <el-table-column align="center" label="专题名" prop="name" />

      <el-table-column align="center" property="iconUrl" label="专题图标">
        <template slot-scope="scope">
          <img v-if="scope.row.iconUrl" :src="scope.row.iconUrl" width="40">
        </template>
      </el-table-column>

      <el-table-column align="center" property="picUrl" label="专题图片">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="级别" prop="level">
        <template slot-scope="scope">
          <el-tag :type="scope.row.level === 'L1' ? 'primary' : 'info' ">{{ scope.row.level === 'L1' ? '一级专题' : '二级专题' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="级别" prop="level">
          <el-select v-model="dataForm.level" @change="onLevelChange">
            <el-option label="一级专题" value="L1" />
            <el-option label="二级专题" value="L2" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="dataForm.level === 'L2'" label="父专题" prop="pid">
          <el-select v-model="dataForm.pid">
            <el-option v-for="item in catL1" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        <el-form-item label="专题图标" prop="iconUrl">
          <el-upload
            :headers="headers"
            :action="uploadPath"
...
<script>
import { ClassTopicList, AddClassTopic, UpdateClassTopic, DeleteClassTopic } from '@/api/classtopic'
import { getL1 } from '@/api/topic'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        name: [{ required: true, message: '专题名不能为空', trigger: 'blur' }],
  ...
  methods: {
    ...
    updateData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          UpdateClassTopic(this.dataForm)
            .then(() => {
              this.getList()
              // 更新L1目录
              this.getCatL1()
