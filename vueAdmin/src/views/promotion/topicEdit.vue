<template>
  <div class="app-container">
    ...
    <el-form ref="topic" :rules="rules" :model="topic" status-icon label-position="left" label-width="100px" style="width: 800px; margin-left:50px;">
      ...
      <el-form-item label="所属父类目">
        <!-- <el-input v-model="topic.pid" /> -->
        <el-cascader
          v-model="topicCategoryIds"
          :options="options"
...
import { getListL1 } from '@/api/classtopic'
...
export default {
  ...
  methods: {
    handleChange(value) {
      console.log(this.topic.topicCategoryIds)
      this.topic.topicCategoryIds = value
    },
    getTopic() {
      ...
      readTopic({ id: this.id })
        .then(response => {
          ...
          this.topicCategoryIds = response.data.data.topic.topicCategoryIds
